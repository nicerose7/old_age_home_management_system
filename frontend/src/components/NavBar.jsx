...
const Navbar = ({ isLoggedIn, setIsLoggedIn, toggleDrawer }) => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      // Fetch upcoming appointments (next 7 days) and residents in parallel
      ...
      // Process appointments with null checks
      ...
      // Process birthdays with null checks
      const birthdayNotifications = (residentsRes.data || [])
        ...
        .map(resident => {
          ...
          if (daysLeft <= 7) { // Only show birthdays in the next 7 days
            let message = '';
